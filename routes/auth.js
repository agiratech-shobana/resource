...
router.get('/redirect', async (req, res) => {
  try {
    ...
    // Save to DB
    try {
      await createUserIfNotExists(claims.oid, claims.name, email);
    } catch (err) {
      console.error("Error saving user:", err);
    }
    ...
    const token = jwt.sign(
      { oid: claims.oid, name: claims.name, email },
      JWT_SECRET,
      { expiresIn: "1h" }
    );
...
router.get('/user', (req, res) => {
  ...
  jwt.verify(token, JWT_SECRET, (err, decoded) => {
    ...
    res.json(decoded); // { oid, name, email }
  });
});
